...
- 这轮焦点与选中态收口已完成的部分：
  ...
  - 当前行为已对齐资源管理器语义：
    ...
    - 保留键盘锚点
    - 键盘导航仍从锚点继续
    - `F2` 不再对“仅锚点、无显式选中”状态生效
  - 右侧条目宿主已补上独立锚点边框视觉，并与显式选中分离。
  - 左侧 sidebar 点击导航时，右侧内容区会统一清除：
    - 显式选中
    - 键盘锚点
  - 当前已新增最小 `SelectionSurfaceCoordinator`：
    - 统一管理“当前哪个选择宿主处于活动态”
    - 当前已接入 `Sidebar` 与 `PrimaryPane`
    - `SecondaryPane` 标识已预留，为后续双面板接线做准备
  - 左右两侧的选中标识当前都已支持：
    - 活动态使用系统选中色
    - 非活动态使用灰化色
    - 窗口失活时统一灰化
  - sidebar tree 当前已额外补齐：
    - 选中指示与背景在失焦态下不再依赖 hover 才刷新
    - 启动默认打开“此电脑”时，左侧默认选中态使用活动色，而不是初始灰态
